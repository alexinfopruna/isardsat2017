...
function vntd_portfolio_register() {
  ...
  register_taxonomy(
      "project-type", array("portfolio"), array(
    "hierarchical" => true,
    "context" => "normal",
    'show_ui' => true,
...
function vntd_save_portfolio_post_settings() {
  ...
  if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
    ...
    if (isset($_POST["title_disable"]))
      update_post_meta($post->ID, "title_disable", $_POST["title_disable"]);
    if (isset($_POST["tagline"]))
      update_post_meta($post->ID, "tagline", $_POST["tagline"]);
    if (isset($_POST["thumb_lightbox"]))
      update_post_meta($post->ID, "thumb_lightbox", $_POST["thumb_lightbox"]);
    if (isset($_POST["link_type"]))
      update_post_meta($post->ID, "link_type", $_POST["link_type"]);
    if (isset($_POST["home_button"]))
      update_post_meta($post->ID, "home_button", $_POST["home_button"]);
    if (isset($_POST["home_button_link"]))
      update_post_meta($post->ID, "home_button_link", $_POST["home_button_link"]);
    if (isset($_POST["related_work"]))
      update_post_meta($post->ID, "related_work", $_POST["related_work"]);
    if (isset($_POST["post_layout"]))
      update_post_meta($post->ID, "post_layout", $_POST["post_layout"]);
    if (isset($_POST["gallery_images"]))
      update_post_meta($post->ID, "gallery_images", $_POST["gallery_images"]);
    if (isset($_POST["gallery_type"]))
      update_post_meta($post->ID, "gallery_type", $_POST["gallery_type"]);
    if (isset($_POST["post_video_url"]))
      update_post_meta($post->ID, "post_video_url", $_POST["post_video_url"]);
    if (isset($_POST["portfolio_external_url"]))
...
<ul id="filter-group-0" class="filter-group">
       <li class="filter-group-li option-set "><a href="#filter" id="show-all" data-option-value="*" class="filter-group-link select option-set relative normal font-primary uppercase"><?php _e('Show All', 'vntd_north'); ?></a></li>
</ul>
  <ul id="filter-group-1" class="filter-group">
       <li class="filter-group-li">     
          <a href="#filter" id="filter-group-1" data-option-value="filter-group" class="filter-group-link option-set relative normal font-primary uppercase">GROUP-1</a> 
